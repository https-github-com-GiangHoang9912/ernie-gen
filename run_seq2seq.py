...
def evaluate_datasets(
    pyreader, reader, eval_func, data_generator, do_pred=False, suffix="out"
):
    def evaluate_dataset(phase, path, is_pred=False):
        pyreader.set_batch_generator(data_generator(input_file=path, phase=phase))
        data_eval_func = eval_func(
            eval_phase="%s_%s" % (phase, suffix),
            features=reader.get_features(phase),
            is_pred=is_pred,
        )
        return data_eval_func

    eval_data = None

    if args.do_val:
        eval_data = evaluate_dataset("dev", args.dev_set)
    if args.do_test:
        eval_data = evaluate_dataset("test", args.test_set)
    if args.do_pred and do_pred:
        eval_data = evaluate_dataset("pred", args.pred_set, is_pred=True)
...
def main(args, request):
    ...
    if args.do_train:
        ...
        if args.in_tokens:
            ...
            max_train_steps = (
                args.epoch * num_train_examples // args.batch_size // trainers_num
            )
            ...
            try:
                ...
                if steps % skip_steps == 0:
                    ...
                    if args.in_tokens:
                        ...
                        current_epoch = (
                            steps * args.batch_size * trainers_num // num_train_examples
                        )
                        current_example = (
                            steps * args.batch_size * trainers_num % num_train_examples
                        )
    ...
    if args.do_val or args.do_test or args.do_pred:
        ...
        data_response = evaluate(suffix=suffix, do_pred=True)
